...
async function queryBGSAPI(endpoint, verb = 'POST', body = {}) {
  ...
  if (response.status === 502 || response.status === 403 || response.status === 429) {
    // This tells Vercel to kill this execution and sleep for FREE behind the scenes!
    throw new RetryableError(`Temporary failure for ${referenceNumber}. Status: ${response.status}`);
  }
